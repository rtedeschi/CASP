...
class ControlModule {
    public:
        ControlModule();
        ~ControlModule();
        void Run(SOURCE_LANGUAGE, MODULE_ID, CODE_COUNT, CODE_INPUT, ARG_COUNT, FUNCTION_ARGS);
    ...
    private:
        ...
        void Execute(MARKUP_OBJECT, LANGUAGE_DESCRIPTOR_OBJECT, MODULE_ID, ARG_COUNT, FUNCTION_ARGS);
        MODULE_REF ModuleRetrieval(MODULE_ID);
        MODULE_RESPONSE ModuleExecution(MODULE_REF, MARKUP_OBJECT, LANGUAGE_DESCRIPTOR_OBJECT, ARG_COUNT, FUNCTION_ARGS);
